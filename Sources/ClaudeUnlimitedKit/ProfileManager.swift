...
public final class ProfileManager: @unchecked Sendable {
    ...
    /// The `claude-unlimited` executable that launchers call. `nil` makes launchers open the engine directly.
    ...
    func launcherScript(for profile: Profile) -> String {
        ...
        var script = "#!/bin/sh\n# Generated by ClaudeUnlimited. Opens Claude with the \(profile.label) profile.\n"
    ...
    func buildLauncher(for profile: Profile) throws {
        ...
        let info: [String: Any] = [
            "CFBundleIdentifier": "io.github.trukhinyuri.claudeunlimited.launcher.\(profile.id)",
