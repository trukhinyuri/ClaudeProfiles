<p align="center">
  <img src="docs/images/app-icon.png" width="128" alt="ClaudeUnlimited icon">
</p>

<h1 align="center">ClaudeUnlimited</h1>
...
<p align="center">
  <a href="https://github.com/trukhinyuri/ClaudeUnlimited/actions/workflows/ci.yml"><img src="https://github.com/trukhinyuri/ClaudeUnlimited/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
...
<p align="center">
  <img src="docs/images/main-window.png" width="820" alt="ClaudeUnlimited window listing four subscriptions with their usage">
</p>
...
ClaudeUnlimited gives every subscription its own Claude window with its own Dock icon, all open at once. Your Claude Code sessions, settings, skills and memory stay shared, so you can pick up any session in whichever of your subscriptions you choose to work in, including when one of them has used up its limit for now.
...
- **Menu bar and CLI.** Open any subscription from the menu bar, or script it with `claude-unlimited`.
...
ClaudeUnlimited is built for people who pay for more than one Claude subscription and use each of them themselves. It deliberately does **not** try to get around how Anthropic meters usage:

| ClaudeUnlimited does | ClaudeUnlimited does not |
|---|---|
...
Each subscription keeps its own limits, applied by Anthropic as usual; ClaudeUnlimited doesn’t raise any of them. Use only subscriptions that are yours and follow Anthropic’s [Consumer Terms](https://www.anthropic.com/legal/consumer-terms) and [Usage Policy](https://www.anthropic.com/legal/aup). If your plans come from an employer, check that their policies allow this setup. This project is not legal advice.
...
```sh
git clone https://github.com/trukhinyuri/ClaudeUnlimited.git
cd ClaudeUnlimited
make install        # builds and copies ClaudeUnlimited.app to ~/Applications
```

Builds are signed ad hoc on your Mac, so Gatekeeper doesn’t get involved. If you download a prebuilt `.zip` from [Releases](https://github.com/trukhinyuri/ClaudeUnlimited/releases) instead, macOS will ask you to confirm the first launch in **System Settings → Privacy & Security → Open Anyway**.

## Use it

1. Open **ClaudeUnlimited** and click **Add Subscription**.
2. Enter the account’s email and, if you like, change the Dock label and color.
3. A new Claude window opens. Sign in there with that account; the email code is entered right in that window.
...
The binary ships inside the app: `ln -s ~/Applications/ClaudeUnlimited.app/Contents/Helpers/claude-unlimited /usr/local/bin/`.
...
| Each profile’s sign-in and window state | `~/Library/Application Support/ClaudeUnlimited/Profiles/<id>` |
| Profile list and backups | `~/Library/Application Support/ClaudeUnlimited` |
...
ClaudeUnlimited shares session cards only. It doesn’t copy Claude Desktop’s scheduled-task list into profiles, so tasks you created in the main app are listed there only.
...
Profile copies are rebuilt from the new version the next time you open them (or right away with `claude-unlimited refresh`). Your sign-ins stay.
...
```sh
make uninstall
rm -rf ~/Library/Application\ Support/ClaudeUnlimited ~/Applications/Claude\ Profiles
```
...
ClaudeUnlimited is an independent project and is not affiliated with, endorsed by or sponsored by Anthropic. Claude is a trademark of Anthropic, PBC.
