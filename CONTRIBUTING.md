...
- **Test what you change.** `make test` runs the suite; logic belongs in `ClaudeUnlimitedKit`, where it can be tested against a sandboxed home directory.

## Layout

| Path | What |
|---|---|
| `Sources/ClaudeUnlimitedKit` | Profiles, session sharing, reading Claude Desktop data, icons |
| `Sources/ClaudeUnlimited` | SwiftUI app and menu bar |
| `Sources/claude-unlimited` | Command-line tool |
| `Tests/ClaudeUnlimitedKitTests` | Swift Testing suite |
| `scripts/build-app.sh` | Assembles and signs `ClaudeUnlimited.app` |

To take screenshots without real accounts, launch the app with sample data:

```sh
open -n --env CLAUDE_UNLIMITED_DEMO=1 build/ClaudeUnlimited.app
```
